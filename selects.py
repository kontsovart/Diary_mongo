...
def avg_mark_region(chart_type,region,district=None,school_id=None,class_id=None,subject_id=None,topic=None,mark_type=None,date1=None,date2=None, title=None):
    ...
    if chart_type == 'line':
        ...
        mas_avg1 = prepare_data_pie_mark(mark_avg, title=title, oxName="Date", oyName="Mark")
    elif chart_type == "pie_student":
        mas_avg1 = prepare_data_pie_student(mark_avg, title=title, oxName="Date", oyName="Mark")
    elif chart_type == "column":
        mas_avg1 = prepare_data_column(mark_avg, title=title, oxName="Subject", oyName="Quantity")
    return mas_avg1
        ...
        try:
            ...
            if class_id:
                if class_id == stud_class["class"]:
                    ...
                    if chart_type == "line" or chart_type == "pie_mark":
                        avg_mark_subject(stud_class["subjects"],subject_id,mark_type,date1,date2,mark_avg,title,chart_type)
                    elif chart_type == "pie_student" or chart_type == "column":
                        avg_mark_pie_column_student(stud_class["subjects"], subject_id, mark_type, date1, date2,mark_avg, title,
                                                    chart_type)
            else:
                # cur.execute("select marks from student where id=(%s)", (student,))
                # mas_subject = cur.fetchone()[0]
                if chart_type == "line" or chart_type == "pie_mark":
                    avg_mark_subject(stud_class["subjects"], subject_id, mark_type, date1, date2, mark_avg, title,
                                     chart_type)
                elif chart_type == "pie_student" or chart_type == "column":
                    avg_mark_pie_column_student(stud_class["subjects"], subject_id, mark_type, date1, date2, mark_avg, title,
                                         chart_type)
...
def avg_mark_pie_column_student(mas,subject_id,mark_type,date1,date2,mark_avg,title,chart_type):
    mark_avg1 = {}
    avg_mark_subject(mas,subject_id,mark_type,date1,date2,mark_avg1,title,chart_type)
    # if mas_marks:
    #     avg_mark_student = float('{:.2f}'.format(sum(mas_marks)/len(mas_marks)))
    sum_mark = 0
    count_mark = 0
    if chart_type == "pie_student":
        ...
        if avg_mark < 3.5:
            if 3 not in mark_avg:
                mark_avg.update({3:0})
            mark_avg[3] +=1
        if avg_mark >=3.5 and avg_mark <4.5:
...
def prepare_data_pie_mark(mas_avg,title=None,oxName=None,oyName=None):
    mas_avg1 = []
    pprint.pprint(mas_avg)
    for count,item in enumerate(mas_avg):
        info = None
...
def prepare_data_pie_student(mas_avg,title=None,oxName=None,oyName=None):
    series = []
    for item in mas_avg:
        series.append({"sector": item, "size": mas_avg[item]})
    return series

def prepare_data_column(mas_avg,title=None,oxName=None,oyName=None):
    series = {}
    series.update({"titles": [{"title": "C-student", "value": "value1"}, {"title": "B-student", "value": "value2"},
...
if __name__== "__main__":
    ...
    mas = avg_mark_region("column","Башкортостан", district="Area0")
    pprint.pprint(mas)
