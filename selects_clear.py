...
def avg_mark_teacher(chart_type, region, school_id,
    ...
    for area in areas:
        ...
        subjects = None
        for item in mydb['school'].find({'idSchool': school_id}, {'_id': 1, 'subject': 1}):
            subjects = item
        avg_mark_teacher_subject(school_id, subjects["subject"],
...
def prepare_data_day(mas_avg, title=None, oxname=None, oyname=None):
    ...
    for count, item in enumerate(mas_avg):
        ...
        for item1 in sorted(mas_avg[item]):
            ...
            for ii in mas_avg1:
                if item1 == ii['date']:
                    ii.update({f"value{count + 1}": float('{:.2f}'.format(mas_avg[item][item1][0] /
                                                                          mas_avg[item][item1][1]))})
                    avg_mark[info][0] += mas_avg[item][item1][0]
                    avg_mark[info][1] += mas_avg[item][item1][1]
        avg_mark[info] = avg_mark[info][0] / avg_mark[info][1]
...
def prepare_data_week(mas_avg, title=None, oxname=None, oyname=None):
    ...
    for count, item in enumerate(mas_avg):
        ...
        for item3 in mas_avg[item]:
            ...
            avg_mark[info][0] += mas_avg[item][item3][0]
            avg_mark[info][1] += mas_avg[item][item3][1]
        avg_mark[info] = avg_mark[info][0]/avg_mark[info][1]
        ...
        for cnt, item1 in enumerate(sorted_mas):
            ...
            if d <= week_date and cnt + 1 != len(sorted_mas):
                for item4 in mas_avg[item][item1]:
                    if item4 == "mark" and mas_week["lineY"] is None:
                        ...
                        avg_mark[info][0] += mas_avg[item][item1][item4][0]
                        avg_mark[info][1] += mas_avg[item][item1][item4][1]
                    if item4 == 'Был':
                        if mas_week["value1"] is None:
                            mas_week["value1"] = 0
                        minn_visit += mas_avg[item][item1][item4]
                        maxx_visit += mas_avg[item][item1][item4]
            ...
            all_subjects[info].update({'oxName': oxname, 'oyName': oyname,
                                       ...
                                       'series': {"titles": [{'title': 'Был', 'value': 'value1'},
                                                  ...
                                                  'data': data1}})
        percent_visit.update({info: minn_visit / maxx_visit})
        try:
            avg_mark[info] = avg_mark[info][0] / avg_mark[info][1]
        except ZeroDivisionError as e:
            avg_mark[info] = 0
    return [all_subjects, percent_visit, avg_mark]
...
def alerts_compare_mark(sub_big, sub_small):
    alert_bool = []
    # print(sub_big, sub_small)
    for item in sub_small:
        mark = sub_big.get(item)
        if mark is not None:
            # print(item, mark, sub_small[item], mark-sub_small[item])
            if 0 < (mark - sub_small[item]) >= 0.5:
                alert_bool.append({'type': 'error',
                                   'info': 'Low marks '+item})
            elif (mark - sub_small[item]) <= -0.5:
                alert_bool.append({'type': 'error',
...
def alerts_compare_visit(sub_big, sub_small):
    alert_bool = []
    # print(sub_big, sub_small)
    for item in sub_small:
        ...
        if percent is not None:
            ...
            if 0 < (percent - sub_small[item]) >= 0.003:
                alert_bool.append({'type': 'warning',
...
if __name__ == "__main__":
    ...
    # mas = avg_mark_region("line", "Башкортостан", title="")
    # pprint.pprint(mas)

    for i in range(20):
        # mas1 = avg_mark_region("line", "Башкортостан", district="Area0", school_id=i, title="")
        # mas1 = avg_mark_teacher("column_line_mix", "Башкортостан", teacher_id=i, title="")
        mas1 = avg_mark_student("line", i, title="")
        print(i)
        print(alerts_compare_mark(mas[1], mas1[1]))
