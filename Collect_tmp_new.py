...
# TODO: fix dates in the mix-chart
def collect_alerts():
    alerts_area = {}
    alerts_region = {}
    alerts_school = {}
        ...
        for area_key, ar_args in area_alerts.items():
            ...
            if len(alerts_area) == 0:
                for school_key, args in school_alerts.items():
                    ...
                    teacher_attent = []
                    student_attent = []
                    if len(teacher_visits):
                        for teacher in teacher_marks[school_key - count_areas - count_region - 1]:
                            teacher_attent.extend(alerts_compare_mark(region_arg['mark'], teacher))
                        for teacher in teacher_visits[school_key - count_areas - count_region - 1]:
                            teacher_attent.extend(alerts_compare_mark(region_arg['visit'], teacher))
                    try:    # TODO: fix is_school == 60 in collect_tmp)student script
                        for student in student_marks[school_key - count_areas - count_region]:
                            student_attent.extend(alerts_compare_mark(region_arg['mark'], student))
                        for student in student_visits[school_key - count_areas - count_region]:
                            student_attent.extend(alerts_compare_mark(region_arg['visit'], student))
                    except:
                        pass
                    ...
                    for i, course in enumerate(teacher_attent):
                        # TODO: Kostil
                        course.update({'id': i + 1 + 100*counter_school, 'redirect_id': counter_school,
                                       'redirect_type': 'school', 'header': f'Teacher ! {i + 1}'})
                        if counter_school not in alerts_school.keys():
                            alerts_school.update({counter_school: []})
                        alerts_school[counter_school].append(course)
                    for i, course in enumerate(student_attent):
                        # TODO: Kostil
                        course.update({'id': i + 1 + 100*counter_school, 'redirect_id': counter_school,
                                       'redirect_type': 'school', 'header': f'Student ! {i + 1}'})
                        if counter_school not in alerts_school.keys():
                            alerts_school.update({counter_school: []})
                        alerts_school[counter_school].append(course)
                    counter_area += 1
    ...
    if len(alerts_area):
        for t, value in alerts_area.items():
            ...
            pprint.pprint({'id': t + count_areas - 1, 'data': value})
    if len(alerts_school):
        for t, value in alerts_school.items():
            mydb['alerts'].insert_one({'id': t + count_areas + count_region, 'data': value})
